        ...
        <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
          <form method="POST" action="{{ route('admin.menus.update',$menu->id) }}" enctype="multipart/form-data">
            ...
            <div class="sm:col-span-6">
              ...
              <div>
                <img class="w-32 h-32" src="{{Storage::url($menu->image)}}" alt="no image">
              </div>
              <div class="mt-1">
            ...
            <div class="sm:col-span-6">
              ...
              <div class="mt-1">
                <input type="number" min="0.00" max="10000.0" step="0.01" id="price" name="price"
                  class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5 @error('name') border-red-400 @enderror"
                  value="{{$menu->price}}" />
              </div>
              @error('name')
              ...
              <div class="mt-1">
                <select multiple name="categories[]" id="categories" class="form-multiselect block w-full mt-1">
                  ...
                  <option value="{{$category->id}}" @selected($menu->
                    categories->contains($category))>{{$category->name}}
                  </option>
                  @endforeach
                </select>
